    ...
    <div>
      ...
      {showCreateResume && (
        <form onSubmit={createResume} onClick={() => setShowCreateResume(false)} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
          <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-md'>
            <h2 className='text-xl font-bold mb-4 pl-2 pt-2'>Create a Resume</h2>
            ...
            <button className='w-full py-2 bg-indigo-600 text-white rounded hover:bg-violet-700 transition-colors'>Create Resume</button>
      ...
      {showUploadResume && (
        <form onSubmit={uploadResume} onClick={() => setShowUploadResume(false)} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
          <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-md'>
            <h2 className='text-xl font-bold mb-4 pl-2 pt-2'>Upload Resume</h2>
            <input onChange={(e)=>setTitle(e.target.value)} value={title} type='text' placeholder='Enter resume title' className='w-full px-4 py-2 mb-4 focus:border-indigo-600 ring-indigo-600' required />
            <div>
            ...
            <button className='w-full py-2 bg-indigo-600 text-white rounded hover:bg-violet-700 transition-colors'>Upload Resume</button>
      ...
      {editResumeId && (
        <form onSubmit={editTitle} onClick={() => setEditResumeId('')} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
          <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-md'>
            <h2 className='text-xl font-bold mb-4 pl-2 pt-2'>Edit Resume Title</h2>
            <input onChange={(e)=>setTitle(e.target.value)} value={title} type='text' placeholder='Enter resume title' className='w-full px-4 py-2 mb-4 focus:border-indigo-600 ring-indigo-600' required/>
            <button className='w-full py-2 bg-indigo-600 text-white rounded hover:bg-violet-700 transition-colors'>Update</button>
